...
fun SelectAppsForCategory(
    ...
    val allCategoryComponentKeys by viewModel.allCategoryComponentKeys.collectAsStateWithLifecycle()

    SelectAppsForCategory(
        categoryEntry = categoryEntry,
        apps = apps,
        allCategoryComponentKeys = allCategoryComponentKeys,
...
fun SelectAppsForCategory(
    categoryEntry: CategoryEntry?,
    apps: List<App>,
    allCategoryComponentKeys: Set<String>?,
    onUpdate: (title: String, componentKeys: List<String>) -> Unit,
    modifier: Modifier = Modifier,
) {
    var filterNonUniqueItems by remember { mutableStateOf(false) }

    val activeIds = remember(categoryEntry) {
        categoryEntry?.itemComponentKeys ?: emptyList()
    }

    val (positionalItems, activeCount) = remember(apps, activeIds, filterNonUniqueItems, allCategoryComponentKeys) {
        val filtered = apps.filter { app ->
            if (filterNonUniqueItems) {
                val assignedKeys = allCategoryComponentKeys ?: emptySet()
                !assignedKeys.contains(app.key.toString()) || activeIds.contains(app.key.toString())
            } else {
                true
            }
        }
    ...
    val loading = categoryEntry == null || apps.isEmpty()
    ...
    val state = key(loading) {
        rememberPositionalListState(
            ...
            onOrderChange = { newList, newCount ->
                ...
                onUpdate(categoryEntry?.title.toString(), activeKeys)
            },
    ...
    PreferenceScaffold(
        label = if (loading) {
            ...
            stringResource(R.string.x_with_y_count, categoryEntry.title, state.activeCount)
        },
        modifier = modifier,
        actions = {
            if (!loading) {
        ...
        Crossfade(targetState = loading, label = "") { isLoading ->
            if (isLoading) {
                ...
                PositionalCategoryAppListPreference(
                    state = state,
...
@Composable
private fun PositionalCategoryAppListPreference(
